...
# if causal mask is used, it captures information about itself and previous tokens
# otherwise, it's a full attention mask caputring interactions between all tokens
...
# config reference from paligemma 224 model: https://huggingface.co/google/paligemma-3b-pt-224/blob/main/config.json
class PaliGemmaConfig():
    def __init__(self,
        ...
        self.id_encoder_decoder = False
        self.vision_config = SiglipVisionConfig(**vision_config)    # this takes key-value dict and converts it into an object
...
class GemmaRMSNorm(nn.Module):
    ...
    def _norm(self, x):
        ...
        # direct ref: https://github.com/huggingface/transformers/pull/29402
...
def rotate_half(x):
    # build the [-x2, x1, -x4, x3] tensor for the sin part of the positoinal encoding
...
def apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=1):
    ...
    # apply the formula (34) for rotatory embedding
...
class GemmaDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        hidden_states, _ = self.self_attn(      # _ is self_attn_weights
...
class GemmaModel(nn.Module):
    ...
    def forward(self,
        ...
        hidden_states = input_embeds # [batch_size, sequence_length, hidden_size]
        ...
        return hidden_states  # [batch_size, sequence_length, hidden_size]
            
            

# another class for causal language modeling
class GemmaForCausalLM(nn.Module):
    ...
    def get_input_embeddings(self):
        return self.model.get_input_embedding()
...
# this resizes the image embeddings so they can be concatenated with the text embeddings
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(self,
        ...
        # masks need to be expanded to match the dimensions of the final_embedding, otherwise we can't use them in torch.where
        text_mask_expanded = text_mask.unsqueeze(-1).expand(-1, -1, embed_dim)
        image_mask_expanded = image_mask.unsqueeze(-1).expand(-1, -1, embed_dim)
        pad_mask_expanded = pad_mask.unsqueeze(-1).expand(-1, -1, embed_dim)

        # torch.where is simplified ternary operation, if condition is true, it returns the first argument, otherwise the second
        # add text embedding
    ...
    def forward(self,
        ...
        assert torch.all(attention_mask == 1), "Input can't have any padding tokens"
