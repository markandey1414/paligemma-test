...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.num_patches = (self.image_size // self.patch_size) ** 2   # since 2d image
...
class SigLipAttention(nn.Module):
    # multi layer attention config
    # attention heads work independently, allowing for parallel computation
    ...
    def forward(
        ...
        # every other state consists of the same input
        ...
        if attn_weights.size() != (batch_size, self.num_heads, seq_length, seq_length):
            raise ValueError(
                f"Attention weights should be of shape {(batch_size, self.num_heads, seq_length, seq_length)}, but is {attn_weights.shape}"
            )
        ...
        if attn_output.size() != (batch_size, self.num_heads, seq_length, self.head_dim):
            raise ValueError(
                f"Attention output should be of shape {(batch_size, self.num_heads, seq_length, self.head_dim)}, but is {attn_output.shape}"
            )

        attn_output = attn_output.transpose(1, 2).contiguous()
        attn_output = attn_output.reshape(batch_size, seq_length, self.embed_dim)
        # reshape function just concatenates the values
...
class SiglipEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        residual - hidden_states        # save to use later
...
class SiglipVisionTransformer(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.embeddings = SiglipVisionEmbeddings(config)        # another class
...
class SiglipVisionModel(nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        # takes batch of images as input and produces batch of embeddings as output
